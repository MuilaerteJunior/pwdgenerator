namespace PwdGenerator.Core.Rules
{
    public abstract class SimpleRuleApplier
    {
        ...
        protected static IEnumerable<int> GetIndexes(int length, short ruleCount)
        {
        ...
        protected static string ReplaceCharAt(string input, IEnumerable<int> indexes, Stack<char> newChar)
        {
        ...
        protected static string UppercaseCharsAt(string input, IEnumerable<int> indexes)
        {
            if (indexes == null || !indexes.Any())
                return input;
            if (indexes != null && indexes.Any(i => i >= input.Length))
